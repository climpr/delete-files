Describe "Delete-Files.ps1" {
    ...
    Context "When a file matches the path, but not the hash" {
        BeforeAll {
            ...
            $script:res = ./src/Delete-Files.ps1 -Path $script:mockDirectory -ConfigurationPath config.json -Depth 5 -DeleteEmptyDirectories $true
        }
    ...
    Context "When 'DeleteEmptyDirectories' is set to 'false'" {
        BeforeAll {
            New-Item -ItemType Directory -Path "$script:mockDirectory" -Force -Confirm:$false | Out-Null
            New-Item -ItemType Directory -Path "$script:mockDirectory/dirDontDelete" -Force -Confirm:$false | Out-Null
        ...
        It "Should output the correct files and directories" {
            $script:res.DeletedFiles | Should -HaveCount 0
            $script:res.DeletedDirectories | Should -HaveCount 0
        }

        It "Should not delete empty directories" {
            Test-Path "$script:mockDirectory/dirDontDelete" | Should -BeTrue
        }
    }
}